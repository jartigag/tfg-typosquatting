...
# dictFile.json outputFile.json >> logFile.log
...
def check_all(e): #e = {'customer':'TEF','domains':['tef.com','tef.es']}
	...
	for dom in e['domains']:
		...
		end_time = time()
		print("cust %s - [%i/%i]"
			%(e['customer'],e['domains'].index(dom)+1,len(e['domains'])),
			"[-]" if d.ip==[] else "[x]",
			"%s"%(d.domain),
...
if __name__ == '__main__':
	...
	# print results as a json (a list of customers. each of these customers
	# is an array of jsons (domains), actually) to outputFile
		...
		for customer in results:
			...
			for domain in customer:
				if not customer.index(domain)==len(customer)-1:
					print(json.dumps(domain.__dict__, indent=2, sort_keys=True),
						end=",\n",file=f)
				else:
					print(json.dumps(domain.__dict__, indent=2, sort_keys=True),
						end="",file=f)
